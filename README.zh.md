...
- **已修复 [issue #2](https://github.com/Skimmenthal13/winfinder/issues/2)** — 修正文档中的 macOS 要求与发布的应用不一致的问题。0.3.0 适用于搭载 Apple Silicon 或 Intel 的 Mac，最低要求为 macOS 14 Sonoma；不支持 macOS 13。
- **更流畅的文件传输** — 复制和移动在后台执行，显示按项目计算的进度和错误汇总。未完成的副本不会显示为已完成的文件。
- **更可靠的文件操作** — 支持跨窗口剪切粘贴，失败的移动可重试。键盘命令仅作用于当前活动窗口；创建、重命名和删除失败时会显示错误。
- **后台创建 ZIP** — 保留子文件夹路径和现有压缩包，压缩失败时不会留下不完整的压缩包。
- **导航与快捷键** — 新增前进/后退历史记录，支持 `Option+左/右方向键`、`F2` 重命名、新建文件和文件夹快捷键，以及快捷键面板。
- **搜索与排序** — 文件变化时自动刷新当前搜索，取消过时搜索，超过 500 个匹配项时显示提示。刷新和搜索后保留排序，文件夹始终置顶；导航或删除后清除失效的选择。
- **扩展与质量检查** — 改进扩展命令中的文件路径处理，新增 17 项回归测试和通用构建兼容性自动检查。

[下载 0.3.0 并查看发行说明](https://github.com/Skimmenthal13/winfinder/releases/tag/v0.3.0)。应用和 DMG 均已使用 Developer ID 签名并通过 Apple 公证。已检查最低系统版本及两种架构；macOS 14/15 上的直接运行测试及 issue 报告者的确认仍待完成。
...
> 0.3.0 最低要求为 macOS 14。旧版 v0.2.0 和 v0.1.8 DMG 未作更改，仍要求 macOS 26。
